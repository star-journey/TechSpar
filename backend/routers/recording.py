...
def _analyze_recording_background(
    ...
    try:
        ...
        from backend.llm_provider import get_llm, invoke_with_retry
        ...
        # Disable the SDK's own retries so the only retry layer is invoke_with_retry
        # (bounded) — otherwise a 524 fans out into SDK-retries × invoke_with_retry
        # attempts of long calls against the provider.
        recording_timeout = load_user_settings(user_id).recording_analysis_timeout_seconds
        llm = get_llm(user_id, max_retries=0, timeout=recording_timeout)
