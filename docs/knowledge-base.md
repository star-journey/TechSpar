...
* 当前页面支持 **Markdown 文本编辑**，也支持通过「导入文档」直接上传 `.md` / `.txt` / `.pdf` / `.docx` 文件，导入后统一转成 Markdown 存进该领域。
* PDF 需要是文字版；扫描版请先 OCR。
* 导入的文档仍建议做一轮提炼：内容越聚焦、术语越统一，出题和评分的参考质量越高。
* 当前没有“上传后等待激活状态”“勾选绑定知识库”这类流程，导入即生效。
