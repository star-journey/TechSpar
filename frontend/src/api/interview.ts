...
// (e.g. Settings 页据 provider_not_configured 跳首登引导)。
async function throwResponseError(res: Response): Promise<never> {
  const text = await res.text();
  let detail = text;
  let code = "";
  try {
    const j = JSON.parse(text) as { detail?: string; code?: string };
...
async function pollJobPrepJob(
  jobId: string,
  { intervalMs = 1500, timeoutMs = 300000 }: { intervalMs?: number; timeoutMs?: number } = {}
  ...
  for (;;) {
    ...
    if (data.status === "error") throw new Error((data.error as string) || "分析失败，请重试。");
...
export async function previewJobPrep(
  ...
  return { preview: data.preview } as ApiResponse<"/api/job-prep/preview", "post">;
}
...
export async function startJobPrep(
  ...
  delete data.status;
  delete data.error;
  return data as ApiResponse<"/api/job-prep/start", "post">;
}
...
export async function getSessionState(
  sessionId: string
): Promise<ApiResponse<"/api/interview/session/{session_id}", "get">> {
