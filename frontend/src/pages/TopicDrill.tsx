...
export default function TopicDrill() {
  ...
  useEffect(() => {
    ...
    getSettings()
      .then((data) => {
        if (active) {
          const training = (data as { training?: { drill_generation_timeout_seconds?: number } }).training;
          setDrillGenerationTimeoutSeconds(training?.drill_generation_timeout_seconds ?? 300);
        }
      })
      .catch(() => {
        if (active) setDrillGenerationTimeoutSeconds(300);
      });
  ...
  // 出题走后台任务(避免网关读超时),这里轮询任务状态直到拿到题目。
  const waitForStartTask = (taskId: string) => new Promise<StartInterviewResponse>((resolve, reject) => {
    ...
    const poll = async () => {
      ...
      try {
        const task = await getTaskStatus(taskId) as { status?: string; result?: unknown; error?: string };
        if (task.status === "done") {
          clearTimer();
          resolve(task.result as StartInterviewResponse);
